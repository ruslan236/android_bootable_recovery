...
if [ "$ROM" == "secondary" ]; then
   ...
   sed 's|format("ext4", "EMMC", "/dev/block/mmcblk0p20", "0");|run_program("/sbin/system_format.sh", "secondary");|g' -i "$MOUNTPOINT"/rs/$updater_script_path || exit 1

   sed 's|mount("ext4", "EMMC", "/dev/block/mmcblk0p21", "/data");|run_program("/sbin/mount_recovery.sh", "secondary");|g' -i "$MOUNTPOINT"/rs/$updater_script_path || exit 1
...
################################################################
   ...
   sed 's|format("ext4", "EMMC", "/dev/block/mmcblk0p20", "0");|run_program("/sbin/system_format.sh", "tertiary");|g' -i "$MOUNTPOINT"/rs/$updater_script_path || exit 1

   sed 's|mount("ext4", "EMMC", "/dev/block/mmcblk0p21", "/data");|run_program("/sbin/mount_recovery.sh", "tertiary");|g' -i "$MOUNTPOINT"/rs/$updater_script_path || exit 1
